...
send !s !slang !shape !change !tick (!on, !off, !m) = osc
    where
      ...
      oscdata | namedParams slang =
                  oscPreamble ++ concatMap (\(k, v) -> [string (name k), v] )
                                           (Map.assocs m)
              | otherwise = oscPreamble ++ (catMaybes $ map (\x -> Map.lookup x m) (params shape))
      cpsPrefix | cpsStamp shape && namedParams slang =
                    [ string "cps",
                      float (cps change),
                      string "delta",
                      float (logicalOffset - logicalOnset),
                      string "cycle",
                      float cycle ]
...
makeConnection !address !port !slang = do
  ...
  return (\ !shape !change !tick (!on,!off,!m) -> do
             let !m' = if namedParams slang then Just m
                       else applyShape' shape m
